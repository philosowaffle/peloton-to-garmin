...
public class GarminAuthenticationService : IGarminAuthenticationService
{
    ...
    private static readonly Dictionary<String,String> BootstrapQueryParams = new Dictionary<String,String> ()
    {
        {"id", "gauth-widget"},
        {"embedWidget", "true"},
    ...
    private static readonly Dictionary<String,String> CommonQueryParams = new Dictionary<String,String> ()
    {
        {"id", "gauth-widget"},
        {"embedWidget", "true"},
    ...
    public async Task<GarminApiAuthentication> SignInAsync()
    {
        ...
        CookieJar jar = null;
        var userAgent = Defaults.UserAgent_GCM;
        ...
        try
        {
            await _apiClient.InitCookieJarAsync(BootstrapQueryParams, userAgent, out jar);
        }
        ...
        // Get CSRF token
        ////////////////////////////////
        var QueryParams_for_CSRF_Request = new Dictionary<String,String>
        {
            {"id", "gauth-widget"},
            {"embedWidget", "true"},
            {"gauthHost", "https://sso.garmin.com/sso/embed"},
        ...
        try
        {
            var tokenResult = await _apiClient.GetCsrfTokenAsync(QueryParams_for_CSRF_Request, BootstrapQueryParams, jar, userAgent);
        ...
        _logger.Information(String.Format("Obtained CSRF token: {0}", csrfToken));
        
        /////////////////////////////////
        // Send Credentials
        ////////////////////////////////
        var signinPayload = new Dictionary<String,String>()
        {
        ...
        try
        {
            sendCredentialsResult = await _apiClient.SendCredentialsAsync(QueryParams_for_CSRF_Request,
                signinPayload, jar, userAgent);
        }
        ...
        _logger.Information("Garmin signin succeeded.");
    ...
    private async Task<GarminApiAuthentication> CompleteGarminAuthenticationAsync(string loginResult, string userAgent)
    {
        ...
        _logger.Information($"Service Ticket: {ticket}");
        ...
        // Get OAuth1 Tokens
        ///////////////////////////////////////////
        _logger.Information("Getting Oauthv1 request token...");
        var consumerCredentials = await _apiClient.GetConsumerCredentialsAsync();
        ...
        _logger.Information($"Got OAuthV1 request token: {oAuth1Token.Token}");

        ////////////////////////////////////////////
        // Exchange for OAuth2 Token
        ///////////////////////////////////////////
        _logger.Information("now requesting OAuthV2 access token...");
    ...
    private async Task<GarminApiAuthentication> ExchangeOAuth1ForOAuth2Async(OAuth1Token oAuth1Token, ConsumerCredentials consumerCredentials, string userAgent)
    {
        ...
        try
        {
            ...
            _logger.Information("Obtained OAuthV2 access token: {0}...", oAuth2Token.Access_Token.Truncate(32));
    ...
    private async Task<OAuth1Token> GetOAuth1Async(string ticket, ConsumerCredentials credentials, string userAgent)
    {
        ...
        var responseParams = HttpUtility.ParseQueryString(oauth1Response);
        var oAuthToken = responseParams.Get("oauth_token");
        var oAuthTokenSecret = responseParams.Get("oauth_token_secret");
